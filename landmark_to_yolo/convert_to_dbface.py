...
for i in range(len(my_imgfiles)):   #for i in range(2): 
    ...
    with open(save_txt_path, "w") as f:        
        for an in range(len(annInfo)):
            ...
            #annotation[0, 0] = (ptx + wd / 2)  # cx
            #annotation[0, 1] = (pty + ht / 2)  # cy
            annotation[0, 0] = ptx # x
            annotation[0, 1] = pty # y
            annotation[0, 2] = wd  # w
            annotation[0, 3] = ht  # h
            if(3 == val_step):
                val_id = 0
                annotation[0, 4] = segment[val_id]  # l0_x
                annotation[0, 5] = segment[val_id+1]  # l0_y
                annotation[0, 6] = segment[val_id+2]
                val_id += val_step
                annotation[0, 7] = segment[val_id]  # l1_x
                annotation[0, 8] = segment[val_id+1]  # l1_y
                annotation[0, 9] = segment[val_id+2]
                val_id += val_step
                annotation[0, 10] = segment[val_id]  # l2_x
                annotation[0, 11] = segment[val_id+1]  # l2_y
                annotation[0, 12] = segment[val_id+2]
                val_id += val_step
                annotation[0, 13] = segment[val_id]  # l3_x
                annotation[0, 14] = segment[val_id+1]  # l3_y
                ...
                annotation[0, 5] = segment[val_id+1]  # l0_y
                val_id += val_step
                annotation[0, 6] = segment[val_id]  # l1_x
                annotation[0, 7] = segment[val_id+1]  # l1_y
                val_id += val_step
                annotation[0, 8] = segment[val_id]  # l2_x
                annotation[0, 9] = segment[val_id+1]  # l2_y
                val_id += val_step
                annotation[0, 10] = segment[val_id]  # l3_x
                annotation[0, 11] = segment[val_id+1]  # l3_y
