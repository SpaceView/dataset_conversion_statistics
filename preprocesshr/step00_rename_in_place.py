...
if __name__ == "__main__":
    ...
    for img_root in image_roots:
        ...
        for img_id in range(TOT_IMG_NUM):
            ...
            j_name = json_file.replace(pathlib.Path(img_file).stem, t_str)
            os.rename(img_file, t_name)
            
            fjs = open(json_file, 'r')
            dataset = json.load(fjs)
            fjs.close()
            ...
            with open(json_file, "w") as fjs:
                json.dump(dataset, fjs)
                fjs.close()
            
            os.rename(json_file, j_name)
